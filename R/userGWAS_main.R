.userGWAS_main <- function(i, cores, k, n, I_LD, V_LD, S_LD, std.lv, varSNPSE2, order, SNPs, beta_SNP, SE_SNP,
  ...
  if(estimation == "DWLS"){
    ...
    basemodel@Options$control <- list(optim.method = "BFGS")
   ...
   gsem_control <- list(optim.method = "BFGS")

  test <- .tryCatch.W.E(Model1_Results <- sem(Model1, sample.cov = S_Fullrun, estimator = "DWLS", WLS.V = W, sample.nobs = 2, optim.dx.tol = .01, std.lv=std.lv, control = gsem_control))
    }
